...
mod test {
    ...
    fn test_oxlintrc_specifying_plugins_will_override() {
        ...
        let config: Oxlintrc =
            serde_json::from_str(r#"{ "plugins": ["typescript", "unicorn", "react", "oxc", "import", "jsdoc", "jest", "vitest", "jsx-a11y", "nextjs", "react-perf", "promise", "node", "vue", "nestjs"] }"#).unwrap();
        assert_eq!(config.plugins, Some(LintPlugins::all()));

        let config: Oxlintrc = serde_json::from_str(r#"{ "plugins": ["nestjs"] }"#).unwrap();
        assert_eq!(config.plugins, Some(LintPlugins::NESTJS));
